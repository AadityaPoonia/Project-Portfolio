...
**In scope:**
- MySQL databases (OPS, QuickSight)
...
- A database we own (e.g. local Postgres/DuckDB on our EC2), populated by the extraction job on a schedule
...
- Where should the sandbox live — local Postgres, DuckDB, or SQLite on the existing EC2?
