...
```
 production DBs        extraction host (this project)              humans
...
Runs anywhere with Python 3.10+ and network access to your databases — your
laptop, a server, a container.

```bash
cd ~/Downloads/DataAtlas                 # wherever you cloned it
...
```yaml
sources:
  - name: app_postgres         # any unique name; used in --source and the catalog
    engine: postgres           # mysql | postgres | redshift
    env_prefix: APP_DB         # -> reads APP_DB_HOST etc. from .env
...
```
APP_DB_HOST=...    APP_DB_PORT=5432    APP_DB_USER=...
...
- **Everything vs one dataset**: every pipeline command takes
  `--source NAME` (e.g. `--source app_postgres`); without it, all configured
...
- **On a schedule** (the end-state): `crontab -e` and add
  `0 3 * * 1 cd ~/Downloads/DataAtlas && ./refresh.sh >> logs/refresh.log 2>&1`
...
## Running on a remote server (optional)

If your databases are only reachable from a specific network (a bastion,
a server inside a VPC), edit code locally and sync it up:

```bash
rsync -av \
  --exclude '.venv' --exclude '__pycache__' --exclude '.pytest_cache' \
  --exclude 'sandbox' --exclude 'logs' --exclude '.env' \
  ./ USER@SERVER:~/DataAtlas/
```

The excludes matter: never overwrite the server's `.env` (real secrets),
`sandbox/` (extraction history), or `logs/` (audit trail). Rebuild the venv
on the server (never copy `.venv` between machines), and copy the catalog
back to view it: `scp USER@SERVER:~/DataAtlas/catalog.html ~/Downloads/`.

## Troubleshooting

| Symptom | Cause / fix |
|---|---|
| `--dry-run` hangs then times out | this machine can't reach the DB host/port — firewall/routing, not credentials |
...
- [x] Phases 1–5 built and battle-tested against a production Postgres replica
- [ ] Cross-source relationship validation via full-column sketches (today:
      sample overlap only, capped at low confidence)
- [ ] Pluggable secret backends (AWS Secrets Manager / Vault) instead of `.env`
